...
func PetFindAll(ctx *gin.Context) {
	...
	if err != nil {
		...
		if pno != "" && pSize != "" {
			...
			if petName != "" && masterName != ""{
				db.Debug().Where("pet_name = ? AND master_name =  ?",petName,masterName).Order("id desc").Find(&u)
				db.Debug().Where("pet_name = ? AND master_name =?",petName,masterName).Order("id desc").Limit(pageSize).Offset((currentPage - 1) * pageSize).Find(&u2)
			}else if masterName != "" {
				...
				db.Debug().Where("pet_name =  ?", petName).Order("id desc").Find(&u)
				db.Debug().Where("pet_name =  ?", petName).Order("id desc").Limit(pageSize).Offset((currentPage - 1) * pageSize).Find(&u2)				
			}
			p = Table.Page{
